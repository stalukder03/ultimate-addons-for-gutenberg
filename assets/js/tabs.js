...
function uagbTabInit(){
	...
	for ( let item = 0; item < tabWrap.length; item++ ) {
		...
		const tabList = tabWrap[item].querySelectorAll( ".uagb-tabs__panel li:not(.uagb-tabs__active) a" );
...
function anchorTabId() {
	if ( window.location.hash != "" && /^#uagb-tabs__tab\d$/.test( window.location.hash ) ) {
		...
		const tabNum = tabPanel.querySelector( "a.uagb-tabs-list" ).getAttribute( "data-tab" );
