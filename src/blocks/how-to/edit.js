...
class UAGBHowTo extends Component {
	...
	onSelectImage( media ) {
		...
		if ( media.sizes ) {
			const new_img = this.getImageSize( media.sizes );
	...
	render() {
		...
		//Time Labels
		const yearlabel = ( timeInYears > 1 ) ? __( "Years", "ultimate-addons-for-gutenberg" ) : __( "Year", "ultimate-addons-for-gutenberg" );
		const monthlabel = ( timeInMonths > 1 ) ? __( " Months " , "ultimate-addons-for-gutenberg" ) : __( " Month " , "ultimate-addons-for-gutenberg" );
		const daylabel = ( timeInDays > 1 ) ? __( " Days ", "ultimate-addons-for-gutenberg" ) :__( " Day ", "ultimate-addons-for-gutenberg" );
		const hourlabel = ( timeInHours > 1 ) ? __( "Hours ", "ultimate-addons-for-gutenberg" ) :__( " Hour ", "ultimate-addons-for-gutenberg" );
		const minslabel = ( minsValue > 1 ) ? __( " Minutes ", "ultimate-addons-for-gutenberg" ) : __( " Minute ", "ultimate-addons-for-gutenberg" );		
...
export default compose(
	withSelect( ( select, ownProps ) => {
		...
		const y  = ( ownProps.attributes.timeInYears ) ? ( ownProps.attributes.timeInYears ) : 0;
		const m  = ( ownProps.attributes.timeInMonths ) ? ( ownProps.attributes.timeInMonths  ) : 0;
		const d  = ( ownProps.attributes.timeInDays ) ? (  ownProps.attributes.timeInDays ) : 0;
		const h  = ( ownProps.attributes.timeInHours ) ? ( ownProps.attributes.timeInHours ) : 0;

		const minutes = ( ownProps.attributes.timeInMins ) ? ownProps.attributes.timeInMins : ownProps.attributes.time;
		...
		const getChildBlocks = select( "core/block-editor" ).getBlocks( ownProps.clientId );
